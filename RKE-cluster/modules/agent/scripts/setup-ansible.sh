#!/bin/bash
# Setup script for RKE Agent Module - Post-Instance Setup
# This script is designed to run AFTER Terraform creates instances with Ansible pre-installed

set -e

echo "🔧 Setting up RKE Agent Module on instances with pre-installed Ansible..."
...
if [ -f "/opt/ansible-playbook" ]; then
    ...
    echo ""
    echo "📋 To run the agent playbook:"
    echo "   cd $PLAYBOOK_DIR"
    echo "   ansible-playbook -i 'localhost,' -c local rke-agent-playbook.yml \\"
    echo "     --extra-vars \"cluster_name=your-cluster region=your-region\""
    echo ""
    echo "🔍 The playbook will:"
    echo "   - Configure the agent node"
    echo "   - Install Docker and RKE agent"
    echo "   - Join the node to the RKE cluster"
else
