...
echo "Fetching OpenVPN SSH key from Secrets Manager..."
...
echo ""
echo "Connect to OpenVPN server (use server IP from terraform output):"
echo "  ssh -i ~/.ssh/openvpn-ssh-keypair.pem openvpnas@<SERVER_IP>"
